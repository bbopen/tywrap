...
export async function generate(
  ...
  const instance = await tywrap(options);
  const written: string[] = [];
  const warnings: string[] = [];
  const outputDir = options.output?.dir ?? './generated';
  const caching = options.performance?.caching ?? false;
  ...
  for (const entry of Object.entries(modules)) {
    ...
    if (!ir) {
      const pythonPath = options.runtime?.node?.pythonPath ?? 'python3';
      ir = await fetchPythonIr(moduleKey, pythonPath);
    ...
    if (!ir) {
      warnings.push(`No IR produced for module ${moduleKey}`);
      continue;
    }
    ...
    const annotatedJSDoc = Boolean(options.output?.annotatedJSDoc);
 ...
 */
async function fetchPythonIr(moduleName: string, pythonPath: string): Promise<unknown | null> {
...
async function computeCacheKey(
  ...
  const runtimePython = options.runtime?.node?.pythonPath ?? 'python3';
  const keyObject = {
    module: moduleName,
    moduleVersion: moduleConfig?.version ?? null,
    runtime: { pythonPath: runtimePython },
    output: {

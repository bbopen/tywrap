...
function renderClient(
  ...
  const classTypes = calls.classes
    ...
    .join('\n');
  const types = `export type BoundRuntime = Pick<ReturnType<typeof import('tywrap/runtime').getRuntimeBridge>, 'call'>;
...
export function bindRuntime(runtime: BoundRuntime): BoundHandle {
  let disposed = false;
...
export declare function bindRuntime(runtime: BoundRuntime): BoundHandle;
`;
...
export function renderClientBindingPrototype(
  module: PythonModule,
  generator: Pick<CodeGenerator, 'generateModuleDefinition' | 'generateModuleBindingTemplate'>,
  ...
  const generated = generator.generateModuleDefinition(module, annotatedJSDoc);
  const bindingTemplate = generator.generateModuleBindingTemplate(
    module,
    annotatedJSDoc,
    runtimeGetterIdentifier
  );
  if (generated.sourceMap !== undefined || bindingTemplate.sourceMap !== undefined) {
    throw new Error('Client binding prototype does not support source maps');
  }
...
type ${runtimeType} = Pick<ReturnType<typeof import('tywrap/runtime').getRuntimeBridge>, 'call'>;

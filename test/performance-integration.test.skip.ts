    ...
    try {
      ...
      const metrics: PerformanceMetrics = {
        ...
        throughput: options.expectedThroughput
          ? options.expectedThroughput / ((endTime - startTime) / 1000)
          : undefined,
      };
      ...
      console.log(
        `⚡ ${name}: ${metrics.duration.toFixed(2)}ms, ${this.formatBytes(metrics.memoryDelta)} memory`
      );
...
describePerformance('Performance Integration Tests', () => {
  ...
  describe('IR Extraction Performance', () => {
    ...
    it('should benefit from caching on repeated analysis', async () => {
      ...
      const { metrics: firstRun } = await testSuite.measureOperation('analyze_cache_miss', () =>
        testSuite.analyzer.analyzePythonModule(module.content, `${module.name}.py`)
      );

      // Second analysis (cache hit)
      const { metrics: secondRun } = await testSuite.measureOperation('analyze_cache_hit', () =>
        testSuite.analyzer.analyzePythonModule(module.content, `${module.name}.py`)
      );
    ...
    it.skip('should handle large modules without memory leaks', async () => {
      ...
      const { result, metrics } = await testSuite.measureOperation('analyze_large_module', () =>
  ...
  describe('Code Generation Performance', () => {
      ...
      const { result, metrics } = await testSuite.measureOperation(
        ...
        async () => {
          ...
          for (const analysis of analysisResults) {
            const generated = await testSuite.generator.generateModule(analysis.module, {
    ...
    it('should optimize bundle sizes', async () => {
      ...
      const { result, metrics } = await testSuite.measureOperation('optimize_bundles', async () => {
        for (const analysis of analysisResults) {
          const generated = await testSuite.generator.generateModule(analysis.module, {
            moduleName: analysis.module.name || 'unknown',
          });
  ...
  describe.skip('Runtime Bridge Performance', () => {
    ...
    it('should demonstrate connection pooling benefits', async () => {
      ...
      const { metrics } = await testSuite.measureOperation('pooled_calls_performance', async () => {
        const calls = [];
        for (let i = 0; i < 20; i++) {
  ...
  describe('Memory Management', () => {
    it('should not leak memory during intensive operations', async () => {
      ...
      console.log(
        `Memory growth after ${iterations} iterations: ${testSuite.formatBytes(memoryGrowth)}`
      );
    ...
    it('should provide accurate memory profiling', async () => {
      ...
      await profiler.profileOperation('test_operation', async () => {
        const modules = testSuite.generateTestModules(3);
        return Promise.all(
          modules.map(m => testSuite.analyzer.analyzePythonModule(m.content, `${m.name}.py`))
        );
      });
  ...
  describe('End-to-End Performance', () => {
    ...
    it('should maintain performance under load', async () => {
      ...
      console.log(
        `Load test results: ${result1.toFixed(2)}ms (6 ops), ${result2.toFixed(2)}ms (12 ops)`
      );

...
export const RUNTIME_CATALOGUE: readonly CatalogueRow[] = [
  ...
  valuesRow({
    ...
    status: 'LOUD_FAIL',
    currentBehavior: 'The bridge rejects the first unsafe integer before JSON encoding.',
    expected: error(/Unsafe Python integer at result:.*outside the JavaScript safe integer range/i),
  }),
  valuesRow({
    id: 'integer-first-rounded',
    call: 'integer_first_rounded()',
    status: 'LOUD_FAIL',
    currentBehavior: 'The bridge rejects 2^53 + 1 before JSON encoding.',
    expected: error(/Unsafe Python integer at result:.*outside the JavaScript safe integer range/i),
  }),
  valuesRow({
    ...
    status: 'LOUD_FAIL',
    currentBehavior: 'The bridge rejects the signed int64 maximum before JSON encoding.',
    expected: error(/Unsafe Python integer at result:.*outside the JavaScript safe integer range/i),
  }),
  valuesRow({
    ...
    status: 'LOUD_FAIL',
    currentBehavior: 'The bridge rejects the signed int64 minimum before JSON encoding.',
    expected: error(/Unsafe Python integer at result:.*outside the JavaScript safe integer range/i),
  }),
  valuesRow({
    ...
    status: 'LOUD_FAIL',
    currentBehavior: 'The bridge rejects factorial(30) before JSON encoding.',
    expected: error(/Unsafe Python integer at result:.*outside the JavaScript safe integer range/i),
  }),
  valuesRow({
    id: 'empty-values',
  ...
  libraryRow({
    ...
    requires: ['numpy', 'pyarrow'],
    status: 'EXPECTED_OK',
    currentBehavior: 'Arrow float16 storage bits decode to numeric values.',
  ...
  libraryRow({
    ...
    requires: ['torch', 'pyarrow'],
    status: 'EXPECTED_OK',
    currentBehavior: 'The nested ndarray converts float16 tensor storage to numeric values.',
    expected: {
      kind: 'match',

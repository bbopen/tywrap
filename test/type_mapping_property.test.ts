...
describe('TypeMapper - Property-Based Tests', () => {
  ...
  test('all Python types map to valid TypeScript types', () => {
    fc.assert(
      fc.property(pythonTypeArb, (pythonType) => {
        ...
        const validKinds = [
          'primitive', 'array', 'tuple', 'object', 'union', 
          'function', 'generic', 'literal', 'custom'
        ];
  ...
  test('union types preserve all member types', () => {
    fc.assert(
      fc.property(
        fc.array(pythonPrimitiveArb, { minLength: 2, maxLength: 5 }),
        (types) => {
          ...
          return result.kind === 'union' && 
                 (result as any).types.length === types.length;
        }
      ),
      { numRuns: 100, seed: 42 }
    );
          ...
          switch (collectionName) {
            ...
            case 'tuple':
              return result.kind === 'tuple' && 
                     (result as any).elementTypes.length === (itemTypes.length > 0 ? itemTypes.length : 1);
            case 'set':
            case 'frozenset':
      ...
      fc.property(
        ...
        (keyType, valueType) => {
          ...
          return result.kind === 'object' && 
                 (result as any).indexSignature !== undefined;
        }
      ),
      { numRuns: 100, seed: 42 }
    );
  ...
  test('type safety invariants are maintained', () => {
    fc.assert(
      fc.property(pythonTypeArb, (pythonType) => {
        ...
        switch (result.kind) {
          case 'primitive':
            const primitiveNames = ['string', 'number', 'boolean', 'null', 'undefined', 'void', 'unknown', 'never'];
          ...
          case 'tuple':
            return Array.isArray((result as any).elementTypes) && (result as any).elementTypes.length > 0;
          ...
          case 'function':
            return Array.isArray((result as any).parameters) && 
                   (result as any).returnType !== undefined &&
                   typeof (result as any).isAsync === 'boolean';
            
          case 'generic':
            return typeof (result as any).name === 'string' && 
                   Array.isArray((result as any).typeArgs);
      ...
      fc.property(
        ...
        (pythonType) => {
          ...
          if (pythonType.kind === 'literal') {
            return result.kind === 'literal' && 
                   (result as any).value === (pythonType as any).value;
          }

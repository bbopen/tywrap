...
${sortedByTime.slice(0, 10).map(r => 
  `- **${r.name}**: ${r.parseTime.toFixed(2)}ms (${r.linesOfCode} LOC, ${r.throughput.toFixed(0)} LOC/sec)`
).join('\n')}

## Memory Usage (Highest Consumers)
${sortedByMemory.slice(0, 10).map(r => 
  `- **${r.name}**: ${(r.memoryDelta / 1024 / 1024).toFixed(2)} MB (${r.fileSize} bytes)`
).join('\n')}

## Low Throughput Files (Need Optimization)
${sortedByThroughput.slice(0, 10).map(r => 
...
`;
  ...
  it('should benchmark medium Python modules (1-10KB)', async () => {
    try {
      ...
      console.log(`Medium file benchmark: ${result.parseTime.toFixed(2)}ms, ${result.throughput.toFixed(0)} LOC/sec`);
    } catch (error) {
  ...
  it('should benchmark large Python modules (>10KB)', async () => {
    const largeModule = Array(500).fill(`
...
`).join('\n');
    ...
    console.log(`Large file benchmark: ${result.parseTime.toFixed(2)}ms, ${result.throughput.toFixed(0)} LOC/sec, ${(result.memoryDelta / 1024 / 1024).toFixed(2)}MB`);
  }, 30000);

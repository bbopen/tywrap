...
class CodegenProfiler {
  ...
  async benchmarkCodeGeneration(
    ...
    const generated = await this.generator.generateModule(analysisResult.module, {
      moduleName,
      exportAll: true,
    });
  ...
  generateReport(results: CodegenBenchmarkResult[]): string {
    ...
    const avgThroughput =
      results.reduce((sum, r) => sum + r.throughput.linesPerSecond, 0) / totalResults;

    const sortedByTime = [...results].sort(
      (a, b) => b.performance.totalTime - a.performance.totalTime
    );
    const sortedByMemory = [...results].sort(
      (a, b) => b.performance.memoryDelta - a.performance.memoryDelta
    );
...
`;
    ...
    console.log(
      `Simple module: ${result.performance.totalTime.toFixed(2)}ms, ${result.throughput.linesPerSecond.toFixed(0)} lines/sec`
    );
  });

  it('should benchmark complex module with many functions', async () => {
    const complexModule = Array(50)
      .fill(null)
      .map(
        (_, i) => `
def function_${i}(
...
`
    ...
    console.log(
      `Complex module: ${result.performance.totalTime.toFixed(2)}ms, ${result.throughput.functionsPerSecond.toFixed(1)} functions/sec`
    );
  });

  it('should benchmark class-heavy module', async () => {
    const classHeavyModule = Array(20)
      .fill(null)
      .map(
        (_, i) => `
class DataModel${i}:
...
`
    ...
    console.log(
      `Class-heavy module: ${result.performance.totalTime.toFixed(2)}ms, ${result.throughput.classesPerSecond.toFixed(1)} classes/sec`
    );
  });
...
`;
    ...
    console.log(
      `Phase breakdown - Analysis: ${(analysisRatio * 100).toFixed(1)}%, Generation: ${(generationRatio * 100).toFixed(1)}%`
    );
  ...
  it('should generate comprehensive performance report', async () => {
    const testCases = [
      ...
      { name: 'small', code: 'def small(x: int) -> int:\n    return x * 2' },
      {
        ...
        code: Array(10)
          .fill(null)
          .map((_, i) => `def func_${i}(x: int = ${i}) -> int:\n    return x + ${i}`)
          .join('\n'),
      },
      {
        name: 'large',

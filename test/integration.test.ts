...
describe('IR-only integration', () => {
  ...
  it('generate() writes TS for math and contains wrappers', async () => {
    ...
    expect(content).toContain("getRuntimeBridge().call<unknown>('math', 'sqrt'");
  });
...
export interface RuntimeBridge {
  call<T = unknown>(module: string, functionName: string, args: unknown[], kwargs?: Record<string, unknown>, validate?: (result: T) => void): Promise<T>;
}

export declare function getRuntimeBridge(): RuntimeBridge;
export type ReturnSchema = unknown;
export declare function createReturnValidator<T = unknown>(schema: ReturnSchema, callSite: string, definitions?: Readonly<Record<string, ReturnSchema>>): (result: T) => T;
`,
        'utf-8'
      );
      await writeFile(

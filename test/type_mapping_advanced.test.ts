...
describe('TypeMapper - Advanced Type Mapping Validation', () => {
  ...
  describe('Collection Type Mappings', () => {
    ...
    test('maps empty list to Array<unknown>', () => {
      const pythonType: PythonType = {
        kind: 'collection',
        name: 'list',
        itemTypes: [],
    ...
    test('maps empty dict to string index signature with str keys and unknown values', () => {
      const pythonType: PythonType = {
        kind: 'collection',
        name: 'dict',
        itemTypes: [],
  ...
  describe('Preset Type Mappings', () => {
    const stdlibMapper = new TypeMapper({ presets: ['stdlib'] });
    const pandasMapper = new TypeMapper({ presets: ['pandas'] });
    const scipyMapper = new TypeMapper({ presets: ['scipy'] });
    const torchMapper = new TypeMapper({ presets: ['torch'] });
    const sklearnMapper = new TypeMapper({ presets: ['sklearn'] });

    test('maps datetime-like stdlib types to string', () => {
      const pythonType: PythonType = { kind: 'custom', name: 'datetime', module: 'datetime' };
      const result = stdlibMapper.mapPythonType(pythonType) as TSPrimitiveType;

      expect(result).toEqual({ kind: 'primitive', name: 'string' });
    });

    test('maps timedelta to number', () => {
      const pythonType: PythonType = { kind: 'custom', name: 'timedelta', module: 'datetime' };
      const result = stdlibMapper.mapPythonType(pythonType) as TSPrimitiveType;

      expect(result).toEqual({ kind: 'primitive', name: 'number' });
    });

    test('maps Decimal, UUID, and Path to string', () => {
      const decimalType: PythonType = { kind: 'custom', name: 'Decimal' };
      const uuidType: PythonType = { kind: 'custom', name: 'UUID' };
      const pathType: PythonType = { kind: 'custom', name: 'Path' };

      expect(stdlibMapper.mapPythonType(decimalType)).toEqual({
        kind: 'primitive',
        name: 'string',
      } satisfies TSPrimitiveType);
      expect(stdlibMapper.mapPythonType(uuidType)).toEqual({
        kind: 'primitive',
        name: 'string',
      } satisfies TSPrimitiveType);
      expect(stdlibMapper.mapPythonType(pathType)).toEqual({
        kind: 'primitive',
        name: 'string',
      } satisfies TSPrimitiveType);
    });

    test('maps pandas DataFrame to a record or array of records', () => {
      const pythonType: PythonType = { kind: 'custom', name: 'DataFrame', module: 'pandas' };
      const result = pandasMapper.mapPythonType(pythonType) as TSUnionType;

      expect(result.kind).toBe('union');
      const types = result.types;
      const hasRecord = types.some(
        t => t.kind === 'object' && (t as TSObjectType).indexSignature?.keyType.kind === 'primitive'
      );
      const hasArray = types.some(t => t.kind === 'array');
      expect(hasRecord).toBe(true);
      expect(hasArray).toBe(true);
    });

    test('maps pandas Series to an array or record', () => {
      const pythonType: PythonType = { kind: 'custom', name: 'Series', module: 'pandas' };
      const result = pandasMapper.mapPythonType(pythonType) as TSUnionType;

      expect(result.kind).toBe('union');
      const types = result.types;
      const hasRecord = types.some(t => t.kind === 'object');
      const hasArray = types.some(t => t.kind === 'array');
      expect(hasRecord).toBe(true);
      expect(hasArray).toBe(true);
    });

    test('maps scipy csr_matrix to a structured sparse object', () => {
      const pythonType: PythonType = {
        kind: 'custom',
        name: 'csr_matrix',
        module: 'scipy.sparse',
      };
      const result = scipyMapper.mapPythonType(pythonType) as TSObjectType;

      expect(result.kind).toBe('object');
      const propNames = result.properties.map(p => p.name);
      expect(propNames).toContain('format');
      expect(propNames).toContain('data');
      expect(propNames).toContain('indices');
      expect(propNames).toContain('indptr');
    });

    test('maps torch Tensor to a structured tensor object', () => {
      const pythonType: PythonType = { kind: 'custom', name: 'Tensor', module: 'torch' };
      const result = torchMapper.mapPythonType(pythonType) as TSObjectType;
    ...
    test('maps sklearn BaseEstimator to an estimator object', () => {
      const pythonType: PythonType = {
        kind: 'custom',
        name: 'BaseEstimator',
        module: 'sklearn.base',
      };
      const result = sklearnMapper.mapPythonType(pythonType) as TSObjectType;

      expect(result.kind).toBe('object');
      const propNames = result.properties.map(p => p.name);
      expect(propNames).toContain('className');
      expect(propNames).toContain('params');
    });
  });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    test('handles deeply nested empty structures', () => {
      ...
      expect(dictType.kind).toBe('object');
      expect(dictType.indexSignature?.valueType).toEqual({ kind: 'primitive', name: 'unknown' });
    });
  });

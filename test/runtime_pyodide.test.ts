  ...
  return {
    runPython: (code: string) => {
      ...
      if (code.includes('def __tywrap_call')) {
        ...
        globals.set('__tywrap_call_method', vi.fn());
        globals.set('__tywrap_dispose_instance', vi.fn());
      }
      return undefined;
    },
...
describe('Pyodide Runtime Bridge', () => {
  ...
  describe('Class Instantiation', () => {
    ...
    it('should handle basic class instantiation', async () => {
      const mockPyodide = createMockPyodide();
      const mockInstantiateFn = vi.fn().mockReturnValue('handle-1');
    ...
    it('should handle class instantiation with args and kwargs', async () => {
      const mockPyodide = createMockPyodide();
      const mockInstantiateFn = vi.fn().mockReturnValue('handle-2');
      mockPyodide.globals.set('__tywrap_instantiate', mockInstantiateFn);
      mockLoadPyodide.mockResolvedValue(mockPyodide);
    ...
    it('should handle instance method calls', async () => {
      const mockPyodide = createMockPyodide();
      const mockCallMethodFn = vi.fn().mockReturnValue(123);
      mockPyodide.globals.set('__tywrap_call_method', mockCallMethodFn);
      mockLoadPyodide.mockResolvedValue(mockPyodide);

      const result = await bridge.callMethod('handle-1', 'count', [1, 2]);

      expect(mockCallMethodFn).toHaveBeenCalledWith('handle-1', 'count', [1, 2], {});
      expect(result).toBe(123);
    });

    it('should dispose instances', async () => {
      const mockPyodide = createMockPyodide();
      const mockDisposeFn = vi.fn().mockReturnValue(true);
      mockPyodide.globals.set('__tywrap_dispose_instance', mockDisposeFn);
      mockLoadPyodide.mockResolvedValue(mockPyodide);

      await bridge.disposeInstance('handle-1');

      expect(mockDisposeFn).toHaveBeenCalledWith('handle-1');
    });
  });
  ...
  describe('Bootstrap Helpers', () => {
    it('should bootstrap helper functions correctly', async () => {
      ...
      expect(runPythonAsyncSpy).toHaveBeenCalledWith(expect.stringContaining('def __tywrap_call'));
      expect(runPythonAsyncSpy).toHaveBeenCalledWith(
        expect.stringContaining('def __tywrap_instantiate')
      );
      expect(runPythonAsyncSpy).toHaveBeenCalledWith(
        expect.stringContaining('def __tywrap_call_method')
      );
      expect(runPythonAsyncSpy).toHaveBeenCalledWith(
        expect.stringContaining('def __tywrap_dispose_instance')
      );
    });
  });
  ...
  describe('Resource Cleanup', () => {
    it('should destroy PyProxy objects after calls', async () => {
      bridge = new PyodideBridge();
      const mockPyodide = createMockPyodide();
      const argProxy = { destroy: vi.fn() };
      const kwargsProxy = { destroy: vi.fn() };
      mockPyodide.toPy = vi
        .fn()
        .mockImplementation((value: unknown) => (Array.isArray(value) ? argProxy : kwargsProxy));
      const callFn = Object.assign(vi.fn().mockReturnValue(42), { destroy: vi.fn() });
      mockPyodide.globals.set('__tywrap_call', callFn);
      mockLoadPyodide.mockResolvedValue(mockPyodide);

      await bridge.call('module', 'func', [1, 2], { key: 'value' });

      expect(argProxy.destroy).toHaveBeenCalled();
      expect(kwargsProxy.destroy).toHaveBeenCalled();
      expect(callFn.destroy).toHaveBeenCalled();
    });
  });
  ...
  describe('Package Loading', () => {
    ...
    it('should handle package loading errors gracefully', async () => {
      ...
      const mockPyodide = createMockPyodide();
      const loadPackageSpy = vi
        .spyOn(mockPyodide, 'loadPackage')

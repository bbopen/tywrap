...
getRuntimeBridge().call('module', 'something', []);
decodeValue();
`;
  ...
  it('extracts complex exports', () => {
    ...
    expect(exports).toEqual(
      expect.arrayContaining(['defaultFunction', 'MyClass', 'bar', 'local1', '*'])
    );
  });

  it('extracts dependencies', () => {
    const deps = (optimizer as any).extractDependencies(code);
    expect(deps).toEqual(expect.arrayContaining(['runtime-bridge', 'codec', './types', './other']));
  });
});

...
Generated async callables have typed results when their return annotations have
supported value conversions. The in-repo Node and Pyodide bridges await Python
coroutines. For `HttpBridge`, your server must await coroutines before it sends
the result. `HttpBridge` cannot verify a remote server's coroutine support.

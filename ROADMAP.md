...
view of current releases, the next release, and intentionally deferred work.

The scientific codec envelope reference lives in
[docs/codec-envelopes.md](./docs/codec-envelopes.md).
...
engages only above the frame ceiling, so small-payload traffic is unchanged.
Raising `codec.maxPayloadBytes` carries large results, with
...
### v0.7.0: scientific data plane foundation
...
This bumped the IR schema to `0.3.0` (the one breaking change. Regenerate
...
`v0.6.1` is internal-only, with no API, behavior, or wire-protocol changes. It
...
Static-analysis warning count dropped from 14 to 3.
...
- standardized the runtime vocabulary on the four-layer glossary: `*IO` to
  `*Transport`, `IntelligentCache` to `ArtifactCache`,
  `WorkerPool`/`PooledWorker` to `TransportPool`/`TransportLease`, and `marker` to
  `typeTag`. On-the-wire keys are unchanged.
...
- support for more SciPy sparse formats
- HTTP server lifecycle management owned by Tywrap
- app-level HMR beyond Tywrap wrapper regeneration and bridge reload
- an explicit opt-in for full Sklearn model serialization, with no implicit
  `pickle` or `joblib` path
